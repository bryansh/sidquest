...
<Dialog.Root open onOpenChange={(open) => { if (!open) onClose(); }}>
  <Dialog.Portal>
    ...
    <Dialog.Content
      onOpenAutoFocus={(e) => { e.preventDefault(); document.getElementById('confirm-delete-btn')?.focus(); }}
      class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm p-5 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] z-50"
    >
